...
>
> **UPDATE 2 — ENGLISH ONLY:** the client has cancelled all Hebrew. Every deliverable is English only: no Hebrew video version, no EN/HE toggle in the explainer, no RTL, no Heebo. If your task prompt or an earlier version of this brief asks for Hebrew (`?lang=he`, `demo-he.mp4`, "bilingual", "HE column"), that instruction is obsolete — ignore it, do not build it, do not verify it, and do not report its absence as an issue. The Hebrew columns below are kept only for reference and must not be used.
...
## 1. Context
- Client: Roy (California). He is producing the marketing material for a friend who runs a **center for children on the autism spectrum** and wants to sell the product to the **parents of those children**. Brand: **Sowala** (only — there is no other brand word; never write "Piga"). Target market: USA. All materials are in **English only** (the client cancelled the Hebrew versions).
...
| 2 | **WARM & COOL** | Second pocket, separate insert. Chill the reusable gel pack in the fridge, or warm the microwaveable grain insert made for plush — heated by the printed guide, checked by an adult (no wires, no batteries). Thermal and weighted materials never share a pouch. *(Wording updated after research: no consumer insert is truly "temperature-limited", so that phrase is now on the never-say list; the poster copy should be brought in line.)* | כיס שני עם תוסף נפרד: ג׳ל לקירור במקרר, או תוסף לחימום במיקרוגל עם הגבלת טמפרטורה (ללא חוטים וסוללות). | `--warm` coral / `--cool` teal |
...
- Type: **Fredoka** 600–700 for display, **Nunito** 400/600/700 for body. Local font CSS: `assets/fonts/fonts.css` (use a relative `<link>` from your HTML). For the published web explainer also add the Google Fonts `<link>` (families Fredoka, Nunito) as a fallback.
...
### Demo video
- 1920×1080, 30 fps, 45–60 s, H.264 yuv420p, AAC audio (generated soft music bed, −18 dBFS-ish, fade out). English captions burned in. **One version only: `demo-en.mp4`.** (A Hebrew version is NOT wanted; if `demo-he.mp4` or `*-he.html` scene files exist they are obsolete leftovers — do not update or verify them.)
